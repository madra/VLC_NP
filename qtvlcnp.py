...
class Player(QtGui.QMainWindow):
    ...
    def __init__(self, master=None):
        ...
        self.mlp_em = self.mediaplayer.event_manager()
        self.mlp_em.event_attach(vlc.EventType.MediaPlayerMediaChanged, self.cb)
    ...
    def cb(self, event):
        self.media.parse()
        txt = self.media.get_meta(0)
        #txt = self.mediaplayer.audio_get_track_description()
        #self.tweet(txt)
        thread.start_new_thread(self.tweet, (txt,))
        print "cb:", event.type, event.u    

    def tweet(self, txt):
        song_name = txt.replace(".mp3", '').replace('.mp4', '')
        r = self.t.request('statuses/update', {'status': "#NP %s" % song_name})
        print r.status_code
